...
class Algo(Arr):
    def __init__(self, win):
        ...
        self.k = 3
        self.build_points()
        self.centroids = [None for x in range(self.k)]
        self.step = 0
    ...
    def compute_centroids(self):
        ...
        ##clear centroids
        for point in self.points:
            point.cancel_centroid()

        ### Build shell for means
        means = [ [0,0] for x in range(self.k)]
        count = [0, 0, 0]
        ...
        for cluster in means:
            cluster[0] /= len(self.points)
            cluster[1] /= len(self.points)

        ### find the nearest centroids
        for cluster, coord in enumerate(means):
            self.get_nearest(cluster, coord)


    def get_nearest(self, cluster, coord):
        x1, y1 = coord
        nearest = None
        mn = m.inf

        for point in self.points:
            if not point.get_centroid():
                x, y = point.get_arr_coord()
        ...
        nearest.set_cluster(cluster)
        nearest.set_centroid()
        self.centroids[cluster] = nearest
    ...
    def build_points(self):
        ...
        for arr_coord in arr:
            ...
            point = Point(self.win, arr_coord, pixel_coord)
